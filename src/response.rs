...
#[derive(Debug)]
pub struct NewsgroupList {
    pub newsgroups: Vec<NewsgroupInfo>,
}
...
#[derive(Debug, Error)]
pub enum ParseNewsgroupListError {
    #[error(transparent)]
    IoError(#[from] std::io::Error),
    #[error(transparent)]
    InvalidNewsgroup(#[from] ParseNewsgroupInfoError),
    #[error("newsgroup list never terminated")]
    UnfinishedNewsgroupList,
}

pub async fn parse_newsgroup_list<T: AsyncBufRead + Unpin>(lines: &mut Lines<T>) -> Result<NewsgroupList, ParseNewsgroupListError> {
    ...
    match state {
        ListParseState::Parsing => {
            Err(ParseNewsgroupListError::UnfinishedNewsgroupList)
        }
        ListParseState::Complete => {
            Ok(NewsgroupList { newsgroups })
        }
    }
}

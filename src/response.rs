...
pub enum Response {
    ...
    CapabilitiesFollow { code: i32, rest: Option<String> },
    ...
    ///    connection).
    Quit { code: i32, rest: Option<String> },
    ...
    /// Response code 281
    ///    Generated by: AUTHINFO PASS (second stage)
    ///    Meaning: user logged in successfully.
    AuthenticationSucceeded { code: i32, rest: Option<String> },
    ...
    /// Response code 381
    ///    Generated by: AUTHINFO USER (first stage)
    ///    Meaning: send password to authenticate.
    EnterPassword { code: i32, rest: Option<String> },
...
pub enum ParseResponseError {
    ...
    EmptyLine,
    #[error(transparent)]
    InvalidIntegerResponseCode(#[from] std::num::ParseIntError),
    #[error("invalid non-integer response code \"{code}\", line \"{line}\"")]
    NonIntegerResponseCode { code: String, line: String },
    #[error("response expected {expected} argument(s) but got {actual} (line \"{line}\")")]
    InvalidFieldCount {
        expected: usize,
        actual: usize,
        line: String,
    ...
    #[error("response argument type incorrect (field \"{field}\", line \"{line}\")")]
    InvalidFieldType { field: String, line: String },
...
impl FromStr for Response {
    ...
    fn from_str(line: &str) -> Result<Self, Self::Err> {
        ...
        let (code, index) = match line.find(char::is_whitespace) {
            Some(index) => (line[..index].parse::<i32>()?, Some(index)),
            None => (line.parse::<i32>()?, None),
        };

        let rest = index.map(|i| line[i + 1..].to_string());

        match (code, rest) {
            (101, rest) => Ok(Response::CapabilitiesFollow { code, rest }),
            (200, rest) => Ok(Response::ServiceAvailablePostingAllowed { code, rest }),
            (205, rest) => Ok(Response::Quit { code, rest }),
            (211, rest) => {
                let parts: Vec<&str> = match rest.as_ref() {
                    Some(rest) => rest.split_whitespace().collect(),
                    None => {
                        return Err(ParseResponseError::InvalidFieldCount {
                            expected: 4,
                            actual: 0,
                            line: line.to_string(),
                        });
                ...
                let number: u32 = parts
                    .get(0)
                    .ok_or_else(|| ParseResponseError::MissingField {
                        field: "number".to_string(),
                        line: line.to_string(),
                    })?
                    .parse()
                    .map_err(|_| ParseResponseError::InvalidFieldType {
                        field: "number".to_string(),
                        line: line.to_string(),
                    })?;
                let low: u32 = parts
            ...
            (215, rest) => Ok(Response::InformationFollows { code, rest }),
            (281, rest) => Ok(Response::AuthenticationSucceeded { code, rest }),
            (381, rest) => Ok(Response::EnterPassword { code, rest }),
            (code, _) => Err(ParseResponseError::UnknownResponseCode {
                code,
                line: line.to_string(),
            }),
        }
...
impl FromStr for Capability {
    ...
    fn from_str(line: &str) -> Result<Self, Self::Err> {
        ...
        let name = parts_iter
            .next()
            .ok_or_else(|| ParseCapabilityError::EmptyLine)?
            .to_string();
        let params: Vec<String> = parts_iter.map(|s| s.to_string()).collect();

        Ok(Capability { name, params })
    }
}
